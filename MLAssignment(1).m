...
hold on;
 plot(X1(:,1),X1(:,2),'ko');           %Samples of data to compare with boundary
 hold on;
 plot(X2(:,1),X2(:,2),'mx');
 title('Neural network desicion contour ','FontSize', 18);
...
% Xts=701:1000;
% %--------------------------------Prepare parameters---------------------
% %fina_data = xlsread('fin_data.csv');    % Reading the data from an excel file.
% [fina_data,txt,raw] = xlsread('fin_data.csv');
% volume_finantial=fina_data(:,5); 
%     close_pr=fina_data(:,4);                  % fourth column - 'close', will be used for formulating a neural net work
% for n=1:1008
...
% % prediction better 
% volume_finantial=fina_data(:,5); %reading volume from the data matrix
% vol_data=zeros(1008,1);
% for n=1:1008 
%     vol_data(n)=volume_finantial(1008-n+1);    %invert the order so that it goes according to time
% end
